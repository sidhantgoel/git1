export default {
	...
	upsertUpvote: async() => {
		try {
			...
			const { upvote_link, upvote_comment, issue_labels, upvote_status } = upvoteForm;
			const issue_id = Aforce_Issue_Table.selectedRow.id;
			await insert_aforce_issue_upvote.run({ upvote_link, upvote_comment, issue_id, upvote_status });
	...
	getSearchedIssues: () => {
		return merge_fetch_searched_issues.data.map(item => ({
